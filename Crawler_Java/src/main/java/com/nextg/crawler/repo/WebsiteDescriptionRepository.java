...
public interface WebsiteDescriptionRepository extends JpaRepository<WebsiteDescription ,Long> {
    ...
    @Query("SELECT wd FROM WebsiteDescription wd WHERE wd.url LIKE %?1%")
    List<WebsiteDescription> findAllWebsiteContainingUrl(String url);

    @Query("SELECT wd FROM WebsiteDescription wd WHERE wd.price LIKE %?1% OR wd.price LIKE '%thuong luong%' OR wd.price IS NULL OR TRIM(wd.price) = ''")
    List<WebsiteDescription> findAllByContainingPrice(String url);
