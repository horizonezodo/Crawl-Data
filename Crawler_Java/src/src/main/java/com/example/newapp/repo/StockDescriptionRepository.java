...
public interface StockDescriptionRepository extends JpaRepository<StockDescription, Long> {
    ...
    @Query("SELECT sd FROM StockDescription sd WHERE sd.url LIKE %?1% OR sd.career LIKE %?1% OR sd.companyName LIKE %?1% OR sd.stockCode LIKE %?1% OR sd.price LIKE %?1% OR sd.date LIKE %?1%")
    Page<StockDescription> search(String keyword, Pageable pageable);

    @Query("SELECT sd FROM StockDescription sd WHERE (sd.url LIKE %?2%) AND (sd.url LIKE %?1% OR sd.career LIKE %?1% OR sd.companyName LIKE %?1% OR sd.stockCode LIKE %?1% OR sd.price LIKE %?1% OR sd.date LIKE %?1% )")
