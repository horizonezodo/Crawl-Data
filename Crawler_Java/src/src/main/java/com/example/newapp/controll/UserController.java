...
import com.example.newapp.request.GetUserSearchHistoryRequest;
import com.example.newapp.request.UpdateUserInfoRequest;
import com.example.newapp.request.UserSearchHistoryRequest;
...
//            });
    ...
    public ResponseEntity<?> getUserSearchHistory(@RequestParam("email")String email, @RequestParam("catId")Long catId ){
        ...
        log.error("Error getting category with email " + email);
        return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    }
